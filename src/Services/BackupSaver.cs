...
namespace WebUI.Services
{
    ...
    public sealed class BackupSaver
    {
        ...
        public async Task MakeBackupsAsync()
        {
            ...
            string sentryConnectionString = Environment.GetEnvironmentVariable("SENTRY_CONNECTION_STRING") ?? string.Empty;
            ...
            using(SentrySdk.Init(sentryConnectionString))
            {
                try
                {
                    ...
                    string deleteionPeriod = Environment.GetEnvironmentVariable("FILE_DELETION_PERIOD_IN_DAYS") ?? "0";
                    int backupDeletionPeriodInDays = int.Parse(deleteionPeriod);
